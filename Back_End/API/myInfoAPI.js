...
const { getEvaluationList,getUserInfo,getProblemInfo,setEvaluationSuccess,deleteEvaluation } = require("../Models/myInfoModel");
...
router.post("/projectsuccess", async (req, res) => {
    try {
      ...
      const result4 = await deleteEvaluation(req.submit,req.problemNumber,req.userId);
      res.json(result1);
    } catch (error) {
      console.error("API 호출 오류:", error);
      res
