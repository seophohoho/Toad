...
async function getUserInfo(userId='seophohoho') {
  ...
  async function deleteEvaluation(submit='seophohoho',problemNumber=3,userId='testman') {
    try {
      // 데이터베이스 연결 얻기
      const connection = await pool.getConnection(); // pool에서 연결 얻기
      // MySQL 쿼리를 사용하여 사용자 인증
      const query = `delete from toad_user_evaluation where evaluation_user = '${userId}' and problem_number=${problemNumber} and submit_user='${submit}'`;
      const rows = await connection.execute(query);
      console.log(rows);
      // 연결 해제
      connection.release();
...
module.exports = { getEvaluationList, getUserInfo, getProblemInfo,setEvaluationSuccess,deleteEvaluation };