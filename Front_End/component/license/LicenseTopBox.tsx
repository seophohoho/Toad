...
const LicenseTopContent = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: center;
...
const LicenseTopBoxComponent: React.FC = () => {
  return (
    <License_TopBox>
      <LicenseTopContent>
        <LogoComponent/>
...
// SignInScreen 컴포넌트
const LicenseTopScreen: React.FC = () => {
  const [userInfo, setUserInfo] = useState({});

  return (
    <LicenseContainer>
      <LicenseTopBoxComponent />
      {/* 여기에 나머지 회원가입 입력 부분을 추가할 수 있습니다. */}
    </LicenseContainer>
  );
};

export default LicenseTopScreen;
